    ...
    void solve()
    {
        ...
        for(int i = 0;i < n;i++){
            if(sa[i] == 0){
                lcpp[i] = n;
                for(int j = i-1;j >= 0;j--){
                    lcpp[j] = Math.min(lcpp[j+1], lcp[j+1]);
                }
                for(int j = i+1;j < n;j++){
                    lcpp[j] = Math.min(lcpp[j-1], lcp[j]);
                }
                break;
            }
        }
        ...
        ret = radixSort(ret);
        for(int i = p-1;i >= 0;i--){
            int last = st.lastle(isa[ret[i]], lcpp[isa[ret[i]]]-1);
            int first = st.firstle(isa[ret[i]]+1, lcpp[isa[ret[i]]]-1);
            if(first == -1)first = n;
            out.println(lcpp[isa[ret[i]]] + " " + (first-last));
        }
    }
